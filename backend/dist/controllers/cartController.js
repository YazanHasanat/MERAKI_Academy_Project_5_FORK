...
const addToCart = async (req, res) => {
    ...
    try {
        ...
        const existingItem = await pool.query(`SELECT * FROM cart_items WHERE cart_id = $1 AND product_id = $2`, [cart_id, product_id]);
        if (existingItem.rows.length > 0) {
            ...
            await pool.query(`INSERT INTO cart_items (cart_id, product_id, quantity) VALUES ($1, $2, $3)`, [cart_id, product_id, quantity]);
        }
        return res
            .status(200)
...
const getCartByUser = async (req, res) => {
    ...
    try {
        const result = await pool.query(`SELECT cart_items.product_id,
              cart_items.quantity,
              products.title,
              products.price,
              products.image_urls
...
const softDeleteCartById = async (req, res) => {
    const cartId = Number(req.params.id);
    if (isNaN(cartId)) {
        return res.status(400).json({ success: false, message: "Wrong cart id" });
    }
    try {
        const result = await pool.query(`UPDATE cart SET is_deleted = 1 WHERE id = $1 RETURNING *`, [cartId]);
...
const updateCartItemQuantity = async (req, res) => {
    ...
    try {
        const cartResult = await pool.query(`SELECT id FROM cart WHERE user_id = $1 AND is_deleted = 0`, [user_id]);
        ...
        const result = await pool.query(`UPDATE cart_items 
       SET quantity = $1 
...
const deleteCartItem = async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(`DELETE FROM cart_items WHERE cart_id = $1 AND product_id = $2 RETURNING *`, [cart_id, product_id]);
        if (result.rowCount === 0) {
            return res
                .status(404)
                .json({ success: false, message: "Product not found in cart" });
